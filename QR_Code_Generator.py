...
def enter_data():
    ...
    if len(user_input) > 2000:
        ...
        logging.warning(f"Попытка ввода слишком длинных данных: {len(user_input)} символов")
        return

    data = user_input
    logging.info(f"Пользователь успешно ввел данные для QR-кода (длина: {len(user_input)} символов)")
...
def choose_params():
    ...
    if size:
        try:
            ...
            logging.warning("Попытка ввода некорректного размера клетки")
    
    level = input("Уровень коррекции L/M/Q/H (Enter — не менять): ").strip().upper()
    if level in ['L', 'M', 'Q', 'H']:
        if level == 'L': error_level = ERROR_CORRECT_L
        elif level == 'M': error_level = ERROR_CORRECT_M
        elif level == 'Q': error_level = ERROR_CORRECT_Q
        elif level == 'H': error_level = ERROR_CORRECT_H
        print(f"Уровень коррекции обновлен на {level}.")
    elif level:
        print("Недопустимый уровень. Используйте L, M, Q или H.")
        logging.warning(f"Попытка ввода недопустимого уровня коррекции: {level}")
    ...
    if fill_color == back_color:
        ...
        logging.warning("Пользователь установил одинаковые цвета для кода и фона")
...
def get_filename():
    ...
    for ch in user_name:
        if ch in forbidden:
            ...
            logging.warning(f"Попытка сохранения с недопустимым именем: {user_name}")
            return None
...
def generate():
    ...
    try:
        ...
        logging.error(f"Ошибка сохранения: Нет прав на запись или файл '{filename}' занят")
